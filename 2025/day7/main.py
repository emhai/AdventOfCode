...
def rec(content, row, col, splits):

    print(splits)
    for i in range(row, len(content)):
        for j in range(col, len(content[i])):
            if i + 1 >= len(content):
                return 1
            if content[i][j] == "|":
                if content[i + 1][j] == ".":
                    ...
                    content_copy_left = copy.deepcopy(content)
                    content_copy_right = copy.deepcopy(content)
                    if content_copy_right[i + 1][j + 1] != "|":
                        content_copy_right[i + 1][j + 1] = "|"
                        splits += rec(content_copy_right, i + 1, j + 1, splits)

                    if content_copy_left[i + 1][j - 1] != "|":
                        content_copy_left[i + 1][j - 1] = "|"
                        splits += rec(content_copy_left, i + 1, j - 1, splits)
...
def part_2(content):
    ...
    for i in range(len(content)):
        for j in range(len(content[i])):
            ...
            if content[i][j] == "S":
                content[i + 1][j] = "|"
                splits += rec(copy.deepcopy(content), i + 1, j, splits)
